...
export class Step13Component implements OnInit {
  ...
  nextstep(){
     let step_13 = JSON.parse(localStorage.getItem("steps_html"));
     let temp = document.getElementById("content").innerHTML;
      step_13 = step_13 + temp;

    localStorage.setItem("steps_html",JSON.stringify(step_13));
        
      this.router.navigate(['../step-14'], {relativeTo : this.route });
  }
  ...
  ngOnInit() {
      ...
      this.step_13_data = JSON.parse(data.formdata.form_meta[12].value);
      console.log(this.step_13_data);
